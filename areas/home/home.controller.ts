...
export class HomeController {
    ...
    async text(@QueryParam('q') q: string) {
        if(this.properties.length === 0) {
            ...
            this.properties = resp.items.map((i: any) => {
                return {
                    ...
                    price: i.elements.price.value.toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,'),
                    address: `${i.elements.street.value}, ${i.elements.city.value} ${i.elements.state.value} ${i.elements.zip.value}`,
        ...
        if(q) {
            filtered = this.properties.filter((p: any) => p.address.toLowerCase().includes(q.toLowerCase()));
        }
        else {
